    ...
    <div className="container mx-auto px-4 py-8 max-w-[1400px]">
      ...
      <div className="flex flex-col gap-6 mb-8">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-card border border-border rounded-xl p-2 flex-1">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" />
              <Input value={tickers[0]} onChange={(e) => setTickers([e.target.value.toUpperCase(), tickers[1]])} className="pl-9 border-none shadow-none bg-transparent" />
            </div>
            <div className="w-px h-6 bg-border"></div>
            <div className="relative flex-1">
              <Search className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" />
              <Input value={tickers[1]} onChange={(e) => setTickers([tickers[0], e.target.value.toUpperCase()])} className="pl-9 border-none shadow-none bg-transparent" />
            </div>
            <Button onClick={handleCompare} disabled={loading} className="bg-primary hover:bg-primary-hover text-white rounded-lg px-8">
      ...
      {!loading && !advancedData && (
        <div className="flex flex-col justify-center items-center py-20 text-center border border-dashed border-border rounded-2xl bg-card/50">
          ...
          <p className="text-muted-foreground text-sm max-w-md">
            Enter two stock tickers above and click Compare to generate a transparent, data-backed analysis report without opaque AI bias.
          </p>
        </div>
      )}
